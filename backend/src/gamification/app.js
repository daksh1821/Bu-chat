  ...
  try {
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.username && path.includes("/badges")) {
      ...
      if (!badgeId || !BADGES[badgeId.toUpperCase()]) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: "invalid badgeId" }) };
      }
      ...
      if (existing.Item) {
        return { statusCode: 409, headers: corsHeaders, body: JSON.stringify({ message: "badge already awarded" }) };
      }
    ...
    if (method === "GET" && event.pathParameters && event.pathParameters.name && path.includes("/leaderboard")) {
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ leaderboard, timeframe, community })
      };
    }
    ...
    if (method === "GET" && path === "/leaderboard") {
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ leaderboard, metric })
      };
    }
