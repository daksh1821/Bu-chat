...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.postId && path.includes("/report")) {
      ...
      if (!userId || !reason) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: "userId and reason required" }) };
      }
      ...
      return { statusCode: 201, headers: corsHeaders, body: JSON.stringify({ reportId, message: "report submitted" })
      };
    }
    ...
    if (method === "POST" && path.includes("/hide")) {
      ...
      if (!moderatorId) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: "moderatorId required" }) };
      }
      ...
      if (!result.Items || result.Items.length === 0) {
        return { statusCode: 404, headers: corsHeaders, body: JSON.stringify({ message: `${targetType} not found` }) };
      }
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ message: `${targetType} hidden` })
      };
    }
    ...
    if (method === "DELETE" && path.includes("/remove")) {
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ message: `${targetType} removed` })
      };
    }
    ...
    if (method === "POST" && event.pathParameters && event.pathParameters.name && path.includes("/ban")) {
      ...
      if (!userId || !moderatorId) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: "userId and moderatorId required" }) };
      }
    ...
    if (method === "DELETE" && path.includes("/ban") && event.pathParameters.name && event.pathParameters.userId) {
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ message: "user unbanned" })
      };
    }
    ...
    if (method === "GET" && path.includes("/ban") && event.pathParameters.name && event.pathParameters.userId) {
      ...
      if (!result.Item || result.Item.status !== "active") {
        return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ banned: false })
        };
      }
    ...
    if (method === "POST" && path.includes("/moderators") && event.pathParameters.name) {
      ...
      if (!userId || !assignedBy) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: "userId and assignedBy required" }) };
      }
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ message: "moderator assigned" })
      };
    }
    ...
    if (method === "GET" && path.includes("/reports/pending")) {
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ reports, count: reports.length })
      };
    }
    ...
    if (method === "PUT" && event.pathParameters && event.pathParameters.reportId) {
      ...
      if (!moderatorId || !status) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: "moderatorId and status required" }) };
      }
      ...
      if (!result.Items || result.Items.length === 0) {
        return { statusCode: 404, headers: corsHeaders, body: JSON.stringify({ message: "report not found" }) };
      }
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify({ message: "report updated" })
      };
    }

    // GET MODERATION QUEUE - GET /communities/{name}/moderation/queue
    if (method === "GET" && path.includes("/moderation/queue") && event.pathParameters.name) {
      const community = event.pathParameters.name;
      const limit = parseInt(event.queryStringParameters?.limit || 25);
      const type = event.queryStringParameters?.type || "all"; // reports, spam, removed

      const params = {
        TableName: TABLE,
        KeyConditionExpression: "PK = :pk",
        ExpressionAttributeValues: { ":pk": `COMM#${community}` },
        FilterExpression: "#type IN (:report, :post, :comment)",
        ExpressionAttributeNames: { "#type": "type" },
        Limit: limit
      };

      if (type === "reports") {
        params.FilterExpression = "#type = :report";
        params.ExpressionAttributeValues[":report"] = "report";
      }
    ...
    // AUTO-MOD RULES - POST /communities/{name}/automod
    if (method === "POST" && path.includes("/automod") && event.pathParameters.name) {
      const community = event.pathParameters.name;
      const body = JSON.parse(event.body || "{}");
      const { moderatorId, rules } = body;
      
      if (!moderatorId || !rules) {
        return { statusCode: 400, headers: corsHeaders, body: JSON.stringify({ message: "moderatorId and rules required" }) };
      }
      ...
      const automodRule = {
        ...
        ruleId,
        community,
        rules, // Array of rules with conditions and actions
        createdBy: moderatorId,
        status: "active",
        createdAt: now
      };

      await ddb.send(new PutCommand({ TableName: TABLE, Item: automodRule }));
